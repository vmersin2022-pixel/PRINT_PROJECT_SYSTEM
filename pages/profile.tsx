...
const ProfilePage: React.FC = () => {
  const { user, isSessionLoading, userOrders, loginWithMagicLink, loginWithPassword, signupWithPassword, loginWithGoogle, loginWithVK, logout } = useApp();
  ...
  const handleDeepLinkLogin = () => {
      // 1. Generate Link to Bot
      const botName = 'print_project_shop_bot';
      const startPayload = 'login_web'; 
      // Payload 'login_web' tells the bot we want a website login link
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
      ...
      try {
          ...
          if (msg.includes('User already registered')) msg = 'Пользователь уже зарегистрирован. Войдите.';
          if (msg.includes('Email not confirmed')) msg = 'Почта не подтверждена.';
          if (msg.includes('rate limit')) msg = 'Слишком много попыток. Подождите.';
        ...
        <div className="min-h-screen pt-24 pb-12 bg-white flex flex-col items-center justify-center">
            <div className="container mx-auto px-4 max-w-md w-full">
                ...
                <div className="bg-zinc-50 border border-black p-8 shadow-[8px_8px_0px_rgba(0,0,0,0.1)]">
                    ...
                    {/* --- DEEP LINK TELEGRAM BUTTON --- */}
                    <div className="space-y-4 mb-8">
                        <button 
                            onClick={handleDeepLinkLogin}
                            className="w-full bg-[#24A1DE] hover:bg-[#208YbC] text-white font-jura font-bold py-3 px-4 flex items-center justify-center gap-3 transition-all shadow-sm group relative overflow-hidden"
                        >
                        ...
                        <p className="text-[10px] text-zinc-400 font-mono text-center leading-tight">
                            Откроется приложение Telegram. Нажмите <b>START</b>, и бот пришлет кнопку для автоматического входа.
                        </p>

                        <div className="flex items-center gap-2 justify-center opacity-50">
                            <div className="h-px bg-zinc-300 w-12" />
                            <span className="font-mono text-[9px]">ИЛИ</span>
                            <div className="h-px bg-zinc-300 w-12" />
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <button 
                                ...
                                disabled={loading}
                                className="bg-white border border-zinc-300 hover:border-black text-zinc-800 font-jura font-bold py-2 px-4 flex items-center justify-center gap-2 transition-all text-[10px]"
                            >
                            ...
                            <button 
                                ...
                                disabled={loading}
                                className="bg-[#0077FF] text-white font-jura font-bold py-2 px-4 flex items-center justify-center gap-2 transition-all hover:bg-blue-700 text-[10px]"
                            >
                                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M13.162 18.994c.609 0 1.016-.217 1.016-.577 0-.361-.557-2.327.916-4.223 1.168-1.503 3.053-1.639 3.053-1.639l.235-.005 3.197-.021c.21 0 .524-.047.625.292.059.197-.058.625-1.571 2.378-2.43 2.813-2.817 2.669-2.817 2.669s-.217 2.344 2.397 2.344h1.666c1.666 0 2.222-1.667 2.222-1.667s.125-1.927-2.917-5.125c-3.097-3.255-2.611-2.917 1.028-7.75 2.219-2.948 3.111-4.792 3.111-4.792s.222-.556-.347-.556h-3.472c-.292 0-.444.139-.528.292 0 0-1.25 2.917-3.028 5.917-1.917 3.222-2.694 2.861-2.694 2.861s-.333-.778-.333-2.972c0-3.111.472-4.417-1.333-4.764-1.205-.231-1.339-.239-3.32-.239-2.545 0-4.656.009-5.906.634-.417.208-.736.681-.542.708.236.028 1.056.125 1.444.653.5.681.486 2.208.486 2.208s-.278 4.222-6.389-6.389c-.181-.306-.361-.556-.847-.556H1.931c-.306 0-.486.153-.569.319-.083.25.028.75 2.722 4.5 3.194 4.444 6.833 8.361 13.078 8.361z" /></svg>
                                VK ID
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <span className="font-mono text-[10px] text-zinc-400 uppercase">EMAIL ВХОД</span>
                        <div className="h-px bg-zinc-300 flex-1"></div>
                    </div>
                ...
                {userOrders.length === 0 ? (
                    <div className="border-2 border-dashed border-zinc-200 p-12 text-center">
                        ...
                        <p className="font-mono text-xs text-zinc-400 mt-2">
                            Если вы делали заказы ранее, они должны отобразиться здесь.
                        </p>
                    </div>
                ) : (
