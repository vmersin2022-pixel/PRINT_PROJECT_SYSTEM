...
const ProfilePage: React.FC = () => {
  const { user, isSessionLoading, userOrders, loginWithMagicLink, loginWithPassword, signupWithPassword, loginWithGoogle, loginWithTelegram, logout } = useApp();
  ...
  const handleTelegramAuth = async (tgUser: TelegramUser) => {
      setLoading(true);
      setMessage(null);
      const { error } = await loginWithTelegram(tgUser);
      if (error) {
          console.error("Telegram Login Error:", error);
          let msg = error.message || 'Ошибка входа через Telegram.';
          if (msg.includes('FunctionsFetchError')) {
              msg = 'Ошибка сервера: Функция авторизации не настроена (Edge Function missing).';
          }
            ...
            <div className="container mx-auto px-4 max-w-md w-full">
                ...
                <div className="bg-zinc-50 border border-black p-8 shadow-[8px_8px_0px_rgba(0,0,0,0.1)]">
                    ...
                    <div className="space-y-4 mb-8">
                        {/* FAST LOGIN BUTTON - GOOGLE */}
                        <button 
                            onClick={handleGoogleLogin}
                            disabled={loading}
                            className="w-full bg-white border border-zinc-300 hover:border-blue-600 hover:text-blue-600 text-zinc-800 font-jura font-bold py-3 px-4 flex items-center justify-center gap-3 transition-all"
                        >
